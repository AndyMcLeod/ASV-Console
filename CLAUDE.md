...
### ⚠ THE FREEZE (Andy, 2026-09-26: "The current instance of ASV Console has frozen. This has happened the last few times. Find the error and correct." — and "is it possibly a function of the computer itself? This is not a powerful machine")

**The evidence** (his `logs/asv_20260925-223018.jsonl`): `client:page_stall` 2-6 s on every frame the guard bar read SLOW/HOLD, then ONE of 122,746 ms; the server answered throughout (his console at 8791 read in 1 s while the page was "frozen"). **The measurement** (`scratchpad/time_guard.js`: the real New Castle extract fetched from a throwaway console's `/api/enc`, his route and telemetry lifted from the log): rings of 9020/7469/7034/6391/5457 vertices; `assess` 130 ms, `assess` WITH the deviation search **4,568 ms → "hold, no deviation"**, and the page ran that search on EVERY frame because `settling` clocked only a deviation the rung COMMANDED — a search that found nothing was never clocked. A synthetic 1,435-poly model (small squares) measured 3-18 ms: THE COUNT IS NOT THE COST, THE RING LENGTH IS. **The fix:** (1) `keepouts.js` — a row-band index (`RING_INDEX_MIN_VERTS` 256, `RING_INDEX_BANDS` 128, a WeakMap keyed by the ring array, built on first use) behind `ringInside` / `ringDist`, used by `blocked`, `clearanceM`, `blockedInfo` (so legClear / firstBlockAlong / the escape's scoring too): pinp over the point's band only, nearest-edge outward by band with a per-edge bbox reject and a stop when the next band cannot beat the best; small features walk exactly as before. `tests/keepout_index.js` is the IDENTITY proof (24,500 asks, 0 disagreements; 2 ms vs 127 ms for 2,000 asks). On the real model: the search 4,568 → 116 ms, `assess` 130 → 53 ms. (2) `static/asv.html` — `edgeSearchAt`: `searchDue = now - edgeSearchAt >= EDGE_REASSESS_MS` in `wantEdge`, stamped when a search is asked for; guard_resume 23 (three frames in one instant ask ONCE, a frame 2.1 s later asks again). Worlds that run clearanceGuard learned `edgeSearchAt` (guard_resume, ais_avoid, clearance_guard). ⚠ Not seen live: he must RESTART his console (it served the old page) — watch `client:page_stall` in his next log. ⚠ Still open: 62 amends in 6 min this morning (1.5-4.5 m dog-legs to port, every ~6 s) — the deviation CHATTERS; with the search clocked and cheap it no longer freezes anything, but that hysteresis (`want = buf + margin`) is its own change. ⚠ **THE HOOK'S TIME LIMIT RUNS THROUGH `tests/lib/limit.py` NOW** (found by precommit_hook.py 4 refusing this very commit): MSYS `timeout` killed a hung suite at the limit and RETURNED ~45 s LATER on this machine, python and node alike (measured: the process gone at 7 s, `timeout` back at 47 s; a plain `sleep` 4 s), so the hook runs `python $HOOK_DIR/../tests/lib/limit.py $SUITE_LIMIT_S <runner> <suite>` — Python's own wait, `taskkill /T` for the tree, exit 124 — with GNU timeout the fallback. Found beside the HOOK, not the cwd, because the hook suite runs the real hook in a temp repo. Expect ~1 s of runner start-up per suite on top of the 15-20 min hook.

### ⚠ TWO WATERS ARE NOT A REVERSAL (Andy, 2026-09-25, a screenshot of a red join drawn straight across Fort Point: "why is the transit line running across land? No flyable turn should not cross land or nogo")

The pair loop's reversal gate admits a pair by GEOMETRY (anti-parallel, `acrossM` within `gapSpan` spacings, ends within `2*turnMaxHalf`), and a cove's last run reversing onto the open sea's first run passes every test with the land between the ends — so the ladder was asked for a loop, every loop entered the land, and the refusal branch (written for "the straight line IS clear, the loop is not") drew the pair red across the fort. Now `landBetweenM(Ap, Bp)` (beside `legSafe`: the join walked 1 m at a time at `HOP_HARD_M` 0.5 m against `ko` — charted features, NOT the buffer, NOT `koTurn`'s channels) and `twoWaters = landM > max(2*buffer, spacing)` take the pair OUT of the reversal gate and out of the stagger count, `koHere = ko` for it, and it falls to the hop path (`legSafe` → `routeAround` → lane:false → knot prune), counted as `nTwoWaters` in the readout. A meter of finger pier on the join is NOT two waters (turn_refusal 1-4 keep refusing — the control); a channel never diverts. `tests/turn_refusal.js` 19/19b (a 30 m wall across a 4-line pattern: the pair the gate admits is routed round the wall's end, nothing red, Add to plan not refused, every leg clear); text pins updated in turn_channel 12 and strike_run 16f/16g (`if(antiParallel && !twoWaters){`). ⚠ Not run live — his survey box is page state I cannot read; the harness reproduces the class.

### ⚠ AIS CONTACTS ARE KEEP-OUTS, AND THE SURVEY COMES BACK ON ITS OWN (Andy, 2026-09-25: "Treat AIS targets with dimensions as NOGO areas. They may be moving so the calculation must update with them. If the AIS target moves through a survey area it will not force a survey pattern reconfiguration. But the ASV will avoid the AIS target and return to the survey line once it is clear with a 100m backup over previously run line. AIS targets without dimensions are to be assumed 20m long and 8m wide." — then "disregard the 20m perimeter buffer")

**The model** (`static/js/ais_keepout.js`, pure): each contact → a `{ring, bb, kind}` polygon in the keep-out frame — `hullBox` (A/B/C/D about the antenna) or 20 x 8 m assumed; heading, else COG while under way, else a disc of the half-diagonal; DEAD-RECKONED by SOG/COG for `age + (now - polledAt)` capped at `AIS_DR_MAX_S` 60 s; SWEPT (convex hull of the box now and `HORIZON_S` on) while sog >= 0.5 kn — the motion, NOT a buffer: the hull is the keep-out and only the vessel's buffer applies (his correction). `aisKeepouts()` answers `{polys, stale, note}`: a poll older than `AIS_KO_STALE_S` 40 s (or none) is STALE, an empty model with a reason. Range `AIS_KO_RANGE_M` 3 km, measured at both ends of the sweep.

**The page:** `aisKeepoutsNow()` builds the model every guard frame (`aisKoDrawn`/`aisKoNote`/`aisKoStale`; the blind BANNER once per outage while armed + running, after the feed has had 40 s). `clearanceGuard`: `koAll = nogo.ko + contacts`, `koG = grantFilter(koAll)`; EVERY rung and counterfactual reads `koAll` (the escape too — berth_grant 10c pins `escapeCourse(p, drift, koAll,`); `updateClearance(extra)` measures the readout against it so `c.kind`/the highlight name the contact; the EDGE rung stands down while a contact is within `buf + HORIZON_S*|vel|` (a dog-leg neither returns nor backs up); `aisCaused` = the charted-only ladder reads clear; `aisKind` names her in the banner before the readout can. `nogo.ko` is NEVER touched — no pattern reconfiguration. **The return:** the hold/helm rungs call `aisAvoidOpen(c, a, aisKind)` after `markGuardHeld` (needs a banked survey: a Go-To gets today's bar); `aisReturnTick(p, aisKo, buf, now)` runs above every branch while `act`: needs `guardHeldOffer()` (on station), REFUSES a stale model (a dead feed read as a quiet sea — found by 6c), asks the AIS-only model whether the way back to `resumePointOn(L, mark, 100)` AND the line from there to its end are clear, dwells `AIS_RETURN_DWELL_MS` 4 s, then `resumeHeldSurvey({backM: 100, auto})` → `resumeHeldRun(opts)`: same pause/upload/LOW/Start, but no `guardOverride`, `resumeSlow = !auto`, its own note/banner, `ais_avoid` + `ais_return` client records. `renderHeldBar` appends "holding for <contact> — resumes on its own". **Polling:** `aisWanted() = aisShow || armed`, ONE always-on `setInterval(pollAIS, AIS_POLL_MS 5 s)`; `aisPolledAt` moves only on an answered poll (`d.ok !== false`); `setAIS(false)` keeps the contacts while armed. `drawAisKeepouts` after `drawNogo`.

**Tests:** `tests/ais_keepout.js` (28: the hull, the offsets, orientation, DR + cap, the sweep, NO buffer, range, stale) and `tests/ais_avoid.js` (33: guard_resume's world copied with three edits — the real `updateClearance`, a classList on the stub elements, a steerable `escapeCourse` that records its model; the hold, the episode, the dwell, the 100 m rejoin, no latches, the operator's resume as the CONTROL, a charted hold, a parked contact, the STALE feed, the edge stand-down with a charted twin that DOES deviate, the helm, a Go-To, the readout, the poll gating). `guard_resume.js`'s world learned the names (`aisPolledAt = Infinity` there: always fresh, never stale). 30 sidecar mutations, see the sweep in the commit. ⚠ Copied worlds: any new global read by `clearanceGuard` must be declared in guard_resume.js AND ais_avoid.js.

**FOUND ON THE WATER (two live passes on a throwaway sim console, a synthetic AIVDM contact on UDP — `scratchpad/ais_feed.py`, `--ais-source nmea --ais-nmea udp:PORT --ais http://127.0.0.1:PORT2`):** the hold at 43-44 m off a parked 30 x 8 m workboat, the return 4 s after she cleared the line, the rejoin 100 m back — and then the rest of line 1 flown at 4.0 kn, because the leg from the rejoin point to the line's end matched NO line (`currentLegLine` needs both ends) and read "turning between lines". Now **a remainder of a line is that line** (`onLineM`: ends at a line's end, begins ON it, `LINE_MATCH_M` clear of both ends — survey_transit_roles 15f/15g; ⚠ the leg memo `_legLine` lives in the eval's scope, so each of those checks uses a route of its own LENGTH). Then the second pass showed **the swing onto the rejoined line commanded HIGH** (13.7 kn into a 180): with the leg now the line, accumLineTime's geometry gate withheld it and no turn record was open, so currentActivity fell to "between coverage regions" (transit). `lineSwing` (set by accumLineTime when the leg is a line the hull is not on/aligned with) → currentActivity role `turn` "turning onto line N" (15h). ⚠ `lineSwing` is read by currentActivity EVERY frame: every world that grabs currentActivity or accumLineTime declares it (eight suites did), and every world that grabs currentLegLine needs `onLineM` + `llEN` (survey_order's leg world was the hook's catch).

### ⚠ THE TRANSITS IN THE LINES CARD (Andy, 2026-09-25: "In the line plan card there is data for lines and turns but not transits within a mission. Add that feature.")

`accumLineTime`'s last branch read `// else: approach / inter-region transit -> neither line nor turn` — the one component of a run the card could not account for. Now: `hopSeg` / `curHop` (a record per hop, opened the first frame off a line whose next is NOT a reversal, closed by the line it leads into — the turn mechanism, one branch down), `approachSec` (no line run yet) and `rthSec` (clocked outside the survey gate while `behavior === "rth"`), all cleared with the line stats. `buildHopTable()` → one row per non-reversal gap: the committed route between the line ends (`hopVia`: the plan's own detour points, matched by the exact coordinates commitPattern wrote) at the TRANSIT speed, and the clocked actual. `lineTableSkeleton(..., ht)` adds a **transits** table (`lt_h*`) under the turns; `transitRowHtml` gained an `actualS` for the approach and RTH rows; the `survey_lines` event and the 30 s `lines` tick carry `transits` (+ `approach_s`, `rth_s`), and playback's Lines card lists them. `tests/hop_timing.js` (6 checks through the real accumLineTime over a two-region plan), `line_table_patch.js` 10 (the rendered table), 8 mutations killed. ⚠ Past ~50 m of stagger a pair is already a hop for the ladder (an older class); here "hop" is `!isReversalGap(k)`, the same test the governor's transit role uses.

### ⚠ PLAYBACK POPULATES EVERY CARD (Andy, 2026-09-25: "Playback renders 7 of ~20 record kinds ... Playback should have all data in the log that a real mission generates ... have playback populate all the cards with all the data collected during the run" — larger files are all right)

**Recorder:** `SessionLogger.STATE_FULL_INTERVAL` 10 s — a full `state` snapshot on the clock, salient or not (the cards' sub-objects ride it: env, water, current, comms, roc, supervisor); the AIS proxy writes an `ais` record at most every `AIS_LOG_INTERVAL_S` (not while quiet). **Page:** `logClient(kind, data)` is the one door; `updateNogoUI` posts `nogo` on change, `recordAction` posts `history` when a row is NEW, `logGuardChange` (2 s ticker) posts `guard` on change, `logLinesTick` (30 s) posts `lines` + the run-time readouts while running; `activity` was already on change. **Playback:** `parseSession` streams `ais` and `client:*` (into `AIS`/`CLIENT`) and puts EVERY other kind on the timeline with `summarize()`; `cardsAt(absT)` resolves each card from the last record at or before the cursor (`lines` vs `survey_lines`: whichever is later; AIS only within `AIS_STALE_S` 60 s); `applyAt` writes the Mission / Lines / AIS / History cards (`m_*`, `l_*`, `a_*`, `h_*`), `render` draws ROC markers and AIS triangles. `tests/playback_cards.js` (26 kinds in, 26 rendered; the cards at the end and mid-run; stale AIS; the digest; the DOM contract), `tests/page_log_posts.js` (each post on change only), `log_routes.py` 2b (the snapshot cadence). **Looked at in the browser pane** on a fresh recording (a throwaway console, a Go-To and an RTH, the live page open so it posted its card data): the Mission card read wind / sea / set / water / nogo / comms / supervisor, History held its rows, the Intent line read HOLDING, and the timeline carried client:nogo / history / activity / guard / page_stall and supervisor_took. ⚠ What the look caught: the 10 s cadence put a `SAFE | Simulator connected` row on the timeline for EVERY snapshot (27 rows for a 3 min replay) — `parseSession` now makes a state row only when autonomy, note or behavior changed from the last row (8 rows; `playback_cards.js` 7).

### ⚠ THE RECORDING GOES QUIET ONCE THE SIM BOAT IS HOME AND IDLE (Andy, 2026-09-25: "once the model is complete and the ASV is back home, wait 10 minutes and stop the simulation so the file does not grow with useless information. Do not log again until a new mission is planned")

Read as: stop the FILE, not the simulation — the console stays connected and ready. `SessionLogger` has `quiet` / `quiet_on()` / `wake()`; `Engine._log_quiet_watch(telem)` runs every sim frame: home (within `LOG_QUIET_HOME_M` 25 m of `home`), not under way (running-and-not-holding is under way; a holding boat in wind station-keeps at over half a knot, so there is NO speed test) for `LOG_QUIET_AFTER_S` 600 s —> `quiet_on` (one `log_quiet` record, then no state/telemetry/`client:*`; commands and lifecycle events still land). `wake()` from `upload`, `start`, `_run_route` (every commanded motion) and a `/api/mission` save with lines or waypoints — one `log_resume` record, the next frame a full snapshot. Sim only (`self._mode != "sim"` returns). `/api/state.log_quiet`; `--log-quiet-s` for harnesses. `tests/log_quiet.py` (7 checks through a real console at 2 s: quiet, dropped page event vs kept command, plan save wakes, Go-To wakes and holding AWAY stays awake, RTH home quiets again). ⚠ Not pinned: the sim-only gate (a real link with no fix never has a home, so the suite cannot see it) — the sweep says so.
...
### ⚠⚠ THE FAR END GIVES FIRST (Andy, 2026-09-24, third pass, over a red pair at Salamander Point: "a slightly curved path from runs 5-6 where the vertex is away from the shallows would resolve the issue")

**Measured over the real New Castle ENC** (throwaway console on 52438 serving `/api/enc`, the turn_refusal world builder driven by a probe over 75 boxes): a pair whose second run started **38 m past** the first run's clipped end was refused at EVERY equal trim, and flew a racetrack the moment the far end alone had given 40 m. Every loop the ladder builds turns around beyond the FURTHER end — which is where the shallows that clipped it are — and equal trims cannot move that (the shorter run runs out first). **Now the run whose end reaches past the other along the pair's own direction gives first, up to the stagger, then both give equally**; the far run may give its whole stub (down to `max(minLine, 10)` m of coverage — that stub is water the pair could not turn at anyway), the equal phase is capped at `TRIM_MAX_M` and a third of what is left; a stagger under 0.5 m is level. ⚠ Past ~50 m of stagger the pair is not a reversal at all (`2*turnMaxHalf`) and ships as a HOP with a point reversal — counted in the hint (`nStagger`), NOT red, an older class this pass did not touch. `turn_refusal.js` 18 (run 3 clipped at both ends to 94 m, 38 m behind run 4, a dock beyond run 4: run 4's start gives 36 m, run 3 nothing, the pair flies) and 18b (the CONTROL: the punch's own ladder-and-judge refuses every equal trim up to run 3's own cap, 27 m). 6 sidecar mutations, 6 killed. The harness world also learned `easeOffered` — with the operator's EASED turn shape the punch used to throw inside the harness (swallowed as "0 turns"); with a 1.0 m approach radius the page declines easing anyway and says so.

### ⚠⚠ ONE JUDGE FOR A JOIN (Andy, 2026-09-24, after the trim rung shipped): "punch out worked, but: 6 corner(s) take the hull inside the 3 m buffer EVEN AT THE LOW SPEED (route waypoints 55, 56, 57, 70, 71, 72) ... The punch out does not seem to be taking into account ALL the buffers."

**Two judges, two speeds.** The punch certified each turn with `turnFlyable` (the guard's projection, at the TURN speed); `doUpload` then walked the whole route with `cornerSlowPlan` (the follower model) **at the plan's FASTEST role speed everywhere** — `planKey` = the fastest of transit/turn/survey = high, 6 kn — and its low-speed second pass cannot shed 6 kn inside a 3 m arc (1.5 kn/s ramp + a 1 s late command), so the arcs of a certified punch came back "unanswered". The 55/56/57 and 70/71/72 groups are two joins.

* **`cornerSlowPlan(..., keyAt)`** — the speed key for the leg INTO each vertex; `planKey` stays the cap (it sizes the corner WINDOW via `cornerReachM`) and the fallback. `doUpload` hands it **`routeSpeedKeys`**: a leg touching a plan `turn` point → turn speed; a leg that IS a committed line (`legIsLine`, the `currentLegLine` match) → survey; else transit. `csKey` now says what was judged instead of `high`.
* **`judgeJoin`** in the page, wrapped round ALL THREE `turnWithRetry` sites in `punchOut` (the ladder, the lead rung, the trim rung): walks `[30 m back, E, ...turn, F, 30 m on]` with **`joinBreaches`** — the SAME breach test as the upload (`cornerBreaches`, factored out of `cornerSlowPlan`) — at the turn speed; if it breaches, again with the join at LOW **flown as pass 2 flies it** (withheld `SPEED_CMD_LATENCY_S`, then ramped) → ships `slow: true` (`turnSlowAt[k]`, the run commands low there); still breaching → `{why: "track"}` and the trim rung answers it. **The window cap is the plan's fastest role speed, exactly as doUpload's** — the first cut used the turn speed and the corner_slow fixture caught the difference.
* **Teeth.** `turn_refusal.js` 16 (the punch's own route walked by the real `cornerSlowPlan`: nothing unanswered, nothing to slow), 17/17b (the WIRING, with a stand-in walk handed to the world — the real walk agrees with the ladder in every world the suite has, so a stand-in is the only way to bite: refuse at turn speed / accept at low → all four joins ship `slow`, asked with the 6 kn window, retry flown late; refuse at both → red as `track`, nothing pulled back). `corner_slow.js` 14b (keyAt), 14c (`routeSpeedKeys` driven + the doUpload wiring pin). **7 sidecar mutations, 7 killed.** `pause_resume.js` learned a null-role `routeSpeedKeys` stub; `strike_run.js`/`turn_refusal.js` harnesses learned the new names (the honest ReferenceError, both times).
* ⚠ NOT the third model: the punch judges against `koTurn` (features at the survey floor + channel turn keep-outs + chart ink), the upload against `nogo.ko`. Same floor by construction today; if they ever differ, 16 is the check that says so.
...
⚠⚠ **OPERATOR ACTIONS on the live console:** restart the server from a tree at or after this commit (`asv.html` is snapshotted at boot; `build` vs `build_on_disk` in `/api/state` says which you are running). Nothing in `roc_config.json` needs editing any more — its `home_id` is ignored, and the Mothership card is still there to select when the ship really is nearby. **CONFIRMED LIVE 2026-09-24 on his console: "rth worked."** The capture/offer/resume chain of the escape incident is still the one thing on this page that has never run live.
