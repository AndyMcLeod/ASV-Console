# ASV Simulator — handoff archive (HANDOFF_ARCHIVE.md)

The handoffs and dated session notes from before 2026-09-05, moved out of `CLAUDE.md` VERBATIM in review #26
(2026-09-15) so every session's start stops paying for them. Nothing here was edited: headings, hashes, quotes and
measurements are as they were written, and so are the pointers inside them - "the handoff above", "START HERE" and the
like mean CLAUDE.md as it stood then. `CLAUDE.md`'s START HERE is the current log; this is the history behind it.
Three spans, in their original order:

1. the 2026-09-04 handoffs through the dated topic sections that followed them (to just before `## Run it`);
2. the dated topic sections from 2026-07-31 to 2026-08-02 that sat between `## Behaviors` and `## Testing notes`;
3. the 2026-07-23 session checkpoint, which ended the file.
