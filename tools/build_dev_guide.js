...
c.push(P("Every regression suite runs on commit when a source it covers is staged. Enable it once per clone; it is versioned in the repository rather than left to each developer to remember. It reports EVERY failure rather than stopping at the first, because two broken invariants are worth knowing about in one pass."));
...
c.push(H2("4.3  Preserving a setting and honouring it are two assertions"));
...
c.push(P("The suite that finally covered it runs the console WITH the feature enabled and cleans up the artefacts it thereby creates. The general form: when a harness disables a subsystem for convenience, list what that mask hides, and make sure at least one suite runs with the mask off."));
...
c.push(P("The instructive part is how easily the WRONG symptom gets fixed. A range guard on the home coordinate was removed as a mutation and the suite crashed rather than failing. The obvious repair was to assert the console was still answering afterwards — and it was: a non-finite coordinate serialises perfectly happily and the state endpoint hands it straight back. That check passed, the suite took the value as its Return-to-Home target, and died four checks later. The invariant that mattered was never \"the console survived\"; it was \"HOME is still a usable coordinate\", because every remaining check steered to it. Name the property the rest of the suite DEPENDS on, then abort through the normal summary path so the failure is scored as a failure rather than as a crash. When a harness dies, fix what killed it — not the first plausible symptom visible from where you are standing."));
