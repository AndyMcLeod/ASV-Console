...
c.push(P("IF A SUPERVISING TAB STOPS RUNNING — minimised for a long time, put to sleep, or crashed — the console says so on its own window and writes it to the session record, and any other tab can take supervision by being opened or clicked. It does NOT stop the boat: a browser hiccup halting a survey mid-line is its own hazard, and nothing the vessel does depends on the page. When such a tab comes back it tells you how long it was gone, because that is the gap you cannot see afterwards."));
