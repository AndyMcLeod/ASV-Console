...
c.push(P("THE TWO GRAPHICS SCALE WITH THE CARD. The tide trace and the wind rose are drawn to fit the card, so making the card bigger makes both of them bigger — they are re-rendered at the new size rather than stretched, and they keep their shape at every size (the rose stays round, the trace keeps its proportions). Drag the card's corner to whatever size you want to read them at; the size is remembered between sessions like every other card."));
