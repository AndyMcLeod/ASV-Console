...
c.push(P("Playback has TWO TABS over one scrubber. CHART replays the sea — the track, the plan (including lines drawn in an EARLIER session: the recorder stores the mission as each session opens), the AIS traffic as it stood at the cursor, and a wind widget. CONTROLS replays the cards — environment, water level, mission, traffic and vessel identity, each showing the last value RECORDED before the cursor. The recorder captures the whole operating picture for this: wind and sea, water level, the traffic on the card, the vessel, and every mission edit — only when something CHANGED, so a quiet hour costs a handful of lines. A recording made before this existed replays with “not recorded” placeholders instead of guesses."));
