...
c.push(P("The console draws seven things that look like lines, and only their COLOR tells them apart: the committed survey lines are YELLOW and are what the vessel runs; the uploaded route is GREEN and is the only green on the chart; the pattern PREVIEW is CYAN and is not in the plan until you press ADD TO PLAN; the survey-area boundary is its own outline; a measurement you drew is MAGENTA; the vessel's trail is blue; and the keep-outs read off the chart are red."));
c.push(P("You do not have to remember that. RIGHT-CLICK A LINE AND CHOOSE \"WHAT IS THIS LINE?\" — the console names the layer it belongs to, in the same words, with what to do about it: a survey line answers with its number, length and heading; the preview answers that it is NOT in the plan and that SURV then RESET clears it; a RED JOIN in the preview answers which two runs it joins, and whether it is a reversal with no flyable turn (Add to plan is refused until it is fixed, 9.5.1) or a hop that Upload will route again; a measurement answers that it is an annotation and never uploaded. A click that is not near anything says so rather than naming the nearest line on the chart."));
...
c.push(NOTE("READ THIS ONE", "If no turn fits clear of the keep-outs, that pair is drawn RED and the console says it has NO FLYABLE TURN — and ADD TO PLAN IS REFUSED until it is fixed. The button grays out, and the note under it names the pairs (right-click a red join to see which runs it joins), what refused each turn, and only the fixes that can work there: draw the box so those lines end short of what refused the turn, or strike off one of the two runs; slow the TURN speed where the vessel could not track the loop; widen the spacing where every turn is a teardrop; keep the lines back from a navigation channel. It never offers a shorter lead, because the console has already tried the pair with no lead at all. Until 16 September 2026 nothing stopped such a plan: it was committed and uploaded with a straight 180° between the two line ends, a reversal in a radius the vessel cannot hold, beside whatever refused the turn."));
c.push(P("STAGGERED REVERSALS ARE NOW FLOWN AS REVERSALS, since 23 September 2026. Two lines that genuinely reverse but whose ENDS are far apart ALONG the line — which is what happens when the chart clips two neighboring runs to different extents — used to be judged as a HOP, so no turn was generated and the boat was asked to come about at a single point. The console now asks how far apart the two lines are ACROSS the track, which is what \"one line spacing apart\" actually means, and separately whether a generated turn can reach between their ends. Both have to be true. The second is what keeps this safe: a pair too far apart for any turn is still a hop, routed as one, rather than being attempted and then refused — which would flag it red and refuse the whole pattern."));
c.push(P("WHAT REMAINS REPORTED, AND IT IS NOT THE SAME THING AS A RED PAIR. Two lines that genuinely reverse, but whose ENDS are far apart ALONG the line rather than across it — which is what happens when the chart clips two neighboring runs to different extents — are judged as a HOP rather than as a reversal, so NO turn is generated between them and the boat is asked to come about at a single point. The punch readout now counts them and says how far back along the previous line the next one starts, because that is what makes one recognizable on the chart: a pair 82 m apart along track and 10 m across looks, at a glance, like an ordinary neighbor."));
c.push(NOTE("WHAT TO DO ABOUT A STAGGERED REVERSAL", "Add to plan is NOT refused — the plan is flyable in the sense that the vessel will attempt it, and on open water she will simply swing wide at that point. Judge it the way you would judge any turn you can see: if the water beside that join is tight, extend the shorter of the two lines so the ends sit abeam of each other, or strike one of the runs off. The console does not widen its own reversal gate to catch these, because doing so would change routes on plans that are running correctly today."));
c.push(P("THE APPROACH IS CHECKED AGAINST THE SAME CHART READING AS THE SURVEY, since 23 September 2026. The console reads structures off the chart IMAGE that the published chart data does not carry — finger piers, float systems — and draws them in magenta. Until that date it only read them over the SURVEY area, so the approach leg from the boat to the first waypoint was planned against published data alone: an unpublished pier between where the boat was lying and where the survey started was invisible to the routing, and the clearance guard, which reads the same model, was equally blind on that leg. Water nobody has read shows up as clear rather than as unknown, so there was no warning and there could not have been one. Upload now reads the chart over the boat AND the whole plan before it routes, the way Go-To, Return-to-Home and a drawn Transit always have. If the boat is already lying in the surveyed water this costs nothing; if it is some way off, Upload may take a moment longer while the chart there is read, and any structure found is bannered and drawn in magenta as usual."));
c.push(P("A RED HOP — two lines that do not reverse, with no detour found between them — does NOT stop Add to plan. Upload routes every hop again with the console's full router, which searches further than Punch Out, and refuses the upload if it still finds no way through."));
c.push(P("When a reversal is refused because its loop would enter a navigation channel, the console says so in those words, the offending channel is OUTLINED on the chart with the blocked spot marked, and the note under Add to plan says how far short of the channel the lines should end — or, if the channel itself is meant to be covered, to draw the survey into it."));
c.push(P("A PUNCH GOES STALE WHEN THE WATER LEVEL MOVES. Punch Out cuts the lines against the chart at the water level in force when you press it, so any change — a tide-station update, or a level you type into `Water m` — throws the punch away: the preview goes back to the pattern as drawn, and Add to plan stays refused, saying so, until you Punch Out again. Adding at that point would otherwise commit the pattern UN-punched — its lines straight through anything in the way, with no turns between them."));
...
c.push(NOTE("A COMMITTED TURN IS NOT CHECKED AGAIN", "A generated turn is a run of waypoints, and the check that refuses a pattern with an unflyable reversal happens at Add to plan only. Clicking one of a turn's points deletes that point and nothing else: the turn you now have is one the console never checked, and deleting all of its points leaves a straight 180° between two line ends. Deleting either line of the pair does not give the turn back. The same goes for a plan added before 16 September 2026, which may carry refused reversals as straight legs. To change a committed survey's turns, CLR PLAN, draw the pattern again, and Punch Out."));
...
c.push(B("BETWEEN TWO LINES the vessel gets a generated turn when the next line is a reversal a few line spacings away (9.5.1), even across a missing line. Anything else — the hop to another region, or two lines at a skew — is a straight leg if that is clear, a detour around the obstacle (amber) if not, and a blocked leg (red) if there is no detour. A red hop does not stop Add to plan: it is routed again at Upload, which refuses the plan if there is still no way through. A red REVERSAL keeps the pattern out of the plan until it is fixed (9.5.1)."));
c.push(P("WITHOUT PUNCH OUT, `Add to plan` commits the lines as drawn: starting at the start corner, alternating, with no generated turns, no leads and nothing clipped. That is never what you get in place of a punch the water level threw away — Add to plan refuses that until you Punch Out again (9.5.1)."));
...
c.push(H2("13.2.1  Corner slowing has lapsed"));
c.push(P("When you Upload, the console walks the routed plan and works out which corners the hull would round INSIDE the keep-out buffer at the survey speed. It banners how many, and then commands the LOW speed at each of those corners by itself as the vessel reaches them — everything else in the plan runs at the speed you set. That promise is tied to THE EXACT PLAN it measured, by the waypoint numbering the vessel reports."));
c.push(P("Four things change the plan underneath it, and all four end the promise: the clearance guard DEVIATING around a hazard, a resume that backs the survey down the line to overlap what the hold missed, a resume of a remainder the guard had held, and any new Upload. The first three put up CORNER SLOWING HAS LAPSED and name which one it was. Nothing is being slowed for a corner after that — the clearance guard is unchanged and still assessing the track ahead, but the corner speeds are gone."));
c.push(NOTE("WHAT TO DO WHEN IT LAPSES", "Set the survey speed selector to LOW to fly the rest of the plan slowly, which is the same speed the corners would have been taken at. The corners can only be MEASURED again by an Upload, so if the run still has a long way to go, Hold, Upload the plan again and Start — the console re-measures against the plan the vessel is actually flying. The console will not guess which corners of an amended route the old numbers name: a slow command at the wrong waypoint is worse than no slow command at all."));
c.push(P("A command the vessel REFUSED does not end the promise, because a refusal means she is still flying the plan the corners were measured on. A command whose reply was LOST does end it: she may have taken it, and the console cannot say which route she is on."));
...
c.push(P("THE LARGEST KNOWN CAUSE OF THAT FREEZE WAS FIXED ON 22 SEPTEMBER 2026, and it is worth knowing what it was, because the symptom was easy to blame on the chart server. The console redrew the ENTIRE chart from scratch on every telemetry frame and again on every single movement of the mouse — and a redraw means projecting every charted feature in view, which in Portsmouth Harbor is around 5,700 features and 480,000 points. Measured on a real plan, one redraw took 64 ms and the console was spending a third of its time doing nothing but that, with nobody touching it; dragging a survey corner asked for a fresh one per mouse movement, which is where the twenty-second freezes came from. The charted layer is now drawn once and reused until something it depends on actually changes — the view, the chart data, the water level, your depth floor or the vessel — and a drag asks for at most one redraw per screen frame. If you still see a freeze, the recording still captures it and it is worth reporting: the remaining causes are different ones."));
