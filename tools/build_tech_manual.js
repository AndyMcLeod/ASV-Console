...
const GUARDS = {
  "coast.js":
    "Coming in on the drift — where the propulsion stops so the vessel arrives at a berth with the way off her, and whether that is safe to attempt. The hull is described by a single coast length, which makes the distance to halve speed the same from any release speed; that invariant is what distinguishes a real coast from the engine-governed ramp, since a constant deceleration would scale the distance as the square of the release speed. The release point is solved backwards in closed form and vectorially, so a set across the approach is handled honestly rather than as an along-track approximation, and the heading is always the ROUTE'S — an earlier design let the solver choose the heading that best stemmed the set, which placed the release point up-set of the berth and, at the operating base that motivated the work, forty metres inside a wharf. The drift track is walked against the same keep-out model the router used, and walked past the berth by the certified clear water, because a hull that carries further than modelled arrives with way still on. Above all the manoeuvre ends on a SPEED and not on a position: the coast length is an estimate with a third of doubt either way, which over a fifty-metre run is far more than the clear water at a tight berth, so no aim point can be made safe — but ending on a speed means that error moves only where powered control resumes, never how fast she is going when it does.",
  "coast_sim.py":
    "The vessel half of the drift-in: that the way actually comes off under hull drag rather than the engine's ramp, that the coast ENDS, and that a hull with no coast datum does not coast at all. The ending is the one to watch — quadratic drag only asymptotes, so a coast with no speed floor leaves a vessel that stopped short of its berth gliding indefinitely, never arriving, never holding, with the propulsion off; that was a real defect in the first cut of the feature and it was found by driving the manoeuvre rather than by reading it. Also holds that a fresh plan is never mid-coast, that the propulsion-off state reaches the telemetry (the mission card carried a branch for it long before anything could set it), and that a malformed release range is refused in words rather than becoming a server error.",
  "hold_point.js":
  ...
  "end_action.js":
    "That the cards name where the run leaves the vessel: an end-of-plan return-to-home shows from the START of a Go-To, transit or survey; is not promised when the chain cannot fire; and re-arms for a run commanded while already under way. Also the INTENT card: that a plan's reasoning is captured WHEN IT IS COMMITTED and never re-derived, that routed and direct plans are told apart, that a partial Rule 9 lane reads differently from a full one, that legs with no clear detour are counted as unsafe, that a plan built with no keep-out model warns instead of reporting a clear run, that waypoint roles are labelled only from what is known, and that every place the route is cleared drops its reasoning with it. And that SURVEYING is distinguished from TRANSITING: only a vessel on a coverage line is surveying, while the approach, the reversals between lines, region hops, go-tos and returns home are transits - one classifier answering for the cards and for the session log, so continuously-collected data can be segmented later without a second implementation free to disagree. And the vessel marker itself: that it is an isosceles triangle whose apex leads along the line of travel, rotated by the course in radians, that a two-tone livery fills the aft band without ever covering the pointed end, and that the hull colour is read from the vessel profile rather than a table of ids in the page",
  "nogo_readout.js":
...
c.push(TBL(["Block", "Fields", "Drives"], [
  ...
  ["`display`", "optional `hull_color`, `hull_color2`", "The vessel marker's colour on the chart - the hull's own livery. Absent falls back to the chart default, so an older profile is unchanged"],
...
c.push(P("A port can be created FROM A NAME. Posting a name with no position geocodes it, and then - the half that matters - snaps the result to charted water deep enough for the selected hull. A geocoder returns the centre of a TOWN, and a town centre is on land: a well known Alaskan port resolves to a street corner, and the console walks outward through the chart's own depth areas until it finds a berth the vessel can float in. Where no navigable water is charted nearby the port is still created at the place centre, but flagged unverified and carrying the reason - never presented as a berth. A name matching nothing is refused rather than becoming a port at sea."));
