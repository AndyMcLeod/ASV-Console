...
const GUARDS = {
  "buoy_lane.js":
    "The Rule 9 channel lane: which side of a channel the vessel rides, marked and unmarked, both directions, that a lone buoy is not treated as a wall, that the lane fact travels WITH the route it describes, and WHERE THE LANE LETS GO — still at full offset at the final buoy pair, still standing on one channel width past it, held to the end of a charted channel that outruns the buoyage, released rather than carried across the gap to a separate channel, reported as PARTIAL when a stretch went un-laned, and a per-vessel reach override that can only widen the wall search, never narrow it",
  "turn_geometry.js":
